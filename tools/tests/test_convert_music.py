...
class ConvertMusicTests(unittest.TestCase):
    ...
    def test_disabled_redbook_accepts_data_only_cue(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            (root / 'disc.cue').write_text('FILE disc.bin BINARY\n  TRACK 01 MODE2/2352\n    INDEX 01 00:00:00\n')
            config = {
                'paths': {'music_source': '.'},
                'capabilities': {'redbook_audio': {'enabled': False, 'reason': 'XA-ADPCM data track'}},
            }
            result = convert_music.convert(root, config)
            self.assertEqual('not_applicable', result['status'])
            self.assertEqual([], result['tracks'])

    def test_disabled_redbook_rejects_audio_track(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            (root / 'disc.cue').write_text('FILE disc.bin BINARY\n  TRACK 02 AUDIO\n    INDEX 01 00:00:00\n')
            config = {
            ...
            with self.assertRaisesRegex(ValueError, 'disabled but the CUE contains AUDIO'):
                convert_music.convert(root, config)
