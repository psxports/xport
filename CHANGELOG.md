...
## 2026-09-25T20:07:52Z

- Scope: new-project code refresh build configuration
- Compatibility: Compatible
- Changed: Configured generated solution builds with platform `x86`, which maps to the vcxproj `Win32` platform declared by the generated solution
- Upgrade: Change `code_refresh.build.platform` from `Win32` to `x86` in projects generated by the startup revision that produced an invalid solution build configuration
- Automation: project config edit
- Verify: X code_refresh

## 2026-09-25T20:04:40Z

- Scope: first-translation audit context
- Compatibility: Compatible
- Changed: Allowed `query --audit-context` to return bounded original evidence and an explicit unmapped gap for TODO functions that do not yet have an implementation-index row
- Upgrade: No migration; use the normal query gate before creating the first C mapping for a TODO function
- Automation: built-in source context query
- Verify: X query 0xADDRESS --image IMAGE --audit-context on a TODO function with no implementation row

## 2026-09-25T19:55:38Z

- Scope: PSX IDA local ISA normalization
- Compatibility: Compatible
- Changed: Added a bounded post-analysis pass that replaces locally inferred MIPS16 instruction heads with aligned MIPS32 words and fails if any non-word code item remains
- Upgrade: Re-export affected images; no accepted immutable export is modified in place
- Automation: built-in IDA export bootstrap
- Verify: Run startup IDA export and canonicalization against an image that previously produced a local 2-byte split

## 2026-09-25T19:52:48Z

- Scope: PSX IDA evidence export
- Compatibility: Compatible
- Changed: Pinned declared PSX code ranges to MIPS32 before analysis so IDA cannot reinterpret aligned R3000A words as inferred MIPS16 instructions
- Upgrade: Re-export any image whose IDA artifacts contain 2-byte MIPS16 instructions inside a declared PSX code range
- Automation: built-in IDA export bootstrap
- Verify: Run startup IDA export and canonicalization; every exported instruction item must preserve original image bytes in 4-byte MIPS words

## 2026-09-25T18:48:50Z

- Scope: new-project startup automation
- Compatibility: Compatible
- Changed: Added one durable `tools/startup.py` supervisor with monotonic revisioned state, single-worker/resume guards, bounded attention packets, one reusable response file and compact completion; folded project initialization, IDA export/canonicalization/acceptance and Ghidra/PsyQ classification into private stages while retaining shared similarity validation used by established projects
- Upgrade: Use `startup FULL_NAME SHORT_NAME` for new Codex project directories; established IDA export/canonicalization, PsyQ classification and similarity validation shims remain compatible, while removed startup-only stages are no longer independent commands
- Automation: built-in startup supervisor
- Verify: python -B -m unittest tests.test_startup tests.test_project_bootstrap

## 2026-09-25T18:07:51Z

- Scope: agent context routing and project bootstrap
- Compatibility: Compatible
- Changed: Reduced the always-read pipeline and generated project contract to stable invariants, task entrypoints and focused documentation links; added `startup FULL_NAME SHORT_NAME` for the current Codex project directory
- Upgrade: Remove duplicated shared methodology and completed-run history from project AGENTS.md; retain only stable game facts and evidence routes; use `startup` for new Codex project directories
- Automation: marker-only
- Verify: X doctor

## 2026-09-25T16:41:11Z

- Scope: converge supervisor protocol, repair handoff and project bootstrap
- Compatibility: Compatible
- Changed: Added monotonic supervisor states and revisions, self-contained attention packets, manifest-bound submit-repair batching, revision-only observation and automatic provisional/full replay gates; deprecated standalone iterate control
- Upgrade: Use one converge launch until ATTENTION_REQUIRED, patch only the complete issued TODO branch, then submit one exact repair manifest with `X trace_workflow submit-repair NAME --manifest PATH`; do not poll unchanged status or run refresh/replay steps manually
- Automation: script: tools/migrations/converge_supervisor_v1.py
- Verify: X doctor
